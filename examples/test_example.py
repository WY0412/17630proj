...
def test_count_words_non_empty_string():
...
def test_person_greet_with_name():
    person = Person("Dave", 20)
    assert person.greet("Eve") == "Hello Eve, my name is Dave!"

def test_person_greet_without_name():
    person = Person("Frank", 22)
    assert person.greet() == "Hello, my name is Frank!"

def test_person_is_adult_true():
    person = Person("George", 18)
...
def test_process_data_with_filter():
    data = [1, 2, 3, 4, 5]
    assert process_data(data, filter_func=lambda x: x > 3) == [4, 5]
...
def test_process_data_with_filter_and_transform():
    data = [1, 2, 3, 4, 5]
    assert process_data(data, filter_func=lambda x: x > 2, transform_func=lambda x: x * 2) == [6, 8, 10]